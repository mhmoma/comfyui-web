# ComfyUI Web inpaint deps (Anima LLLite v2): check then install missing only. NO pip.
# Required plugins: Anima-LLLite, controlnet_aux, BSS FLSampler, CropAndStitch (small), essentials/MaskFix+ (large)
...
$Plugins = @(
    @{ Id='anima-lllite'; Label='ComfyUI-Anima-LLLite'; Folders=@('ComfyUI-Anima-LLLite'); Git='https://github.com/kohya-ss/ComfyUI-Anima-LLLite.git'; Nodes=@('AnimaLLLiteApply'); Any=$false; Optional=$false; Note='small+large' }
    @{ Id='controlnet-aux'; Label='comfyui_controlnet_aux'; Folders=@('comfyui_controlnet_aux'); Git='https://github.com/Fannovel16/comfyui_controlnet_aux.git'; Nodes=@('InpaintPreprocessor'); Any=$false; Optional=$false; Note='small+large' }
    @{ Id='fls-sampler'; Label='ComfyUI-BSS_FLSampler'; Folders=@('ComfyUI-BSS_FLSampler','ComfyUI_BSS_FLSampler'); Git='https://github.com/BlackSnowSkill/ComfyUI-BSS_FLSampler.git'; Nodes=@('FLS_SamplerV4'); Any=$false; Optional=$false; Note='small+large' }
    @{ Id='inpaint-crop-stitch'; Label='ComfyUI-Inpaint-CropAndStitch'; Folders=@('ComfyUI-Inpaint-CropAndStitch'); Git='https://github.com/lquesada/ComfyUI-Inpaint-CropAndStitch.git'; Nodes=@('InpaintCropImproved','InpaintStitchImproved'); Any=$false; Optional=$false; Note='small only' }
    @{ Id='essentials'; Label='ComfyUI_essentials'; Folders=@('ComfyUI_essentials'); Git='https://github.com/cubiq/ComfyUI_essentials.git'; Nodes=@('MaskFix+'); Any=$false; Optional=$false; Note='large only' }
)
...
foreach ($p in $Plugins) {
    ...
    if ($ready) { Write-Ok "$($p.Label) [$tag]$note" } else { Write-Miss "$($p.Label) [$tag]$note"; $toInstall += $p }
    $report.Add("PLUGIN | $($p.Label) | ready=$ready | optional=$($p.Optional) | note=$($p.Note) | via=$how")
}
